@props([
    ...
    'description' => 'Tidak ada data yang tersedia untuk ditampilkan saat ini.',
    'actionText' => null,
    'actionUrl' => null,
    'colspan' => null,
])
...
@php
    $actionBtnText = $actionText ?? $attributes->get('buttonText');
    $actionBtnUrl = $actionUrl ?? $attributes->get('buttonUrl');
@endphp
...
@else
    <div {{ $attributes->merge(['class' => 'py-12 px-4 text-center']) }}>
@endif

    <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-500 dark:text-indigo-400 mb-4 shadow-xs">
        @if($icon === 'room')
        ...
        @elseif($icon === 'invoice')
            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"></path></svg>
        @elseif($icon === 'payment')
    ...
    <h4 class="text-base font-bold text-gray-900 dark:text-white mb-1">{{ $title }}</h4>
    <p class="text-xs text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-5 leading-relaxed">{{ $description }}</p>

    @if($actionBtnUrl && $actionBtnText)
        <div>
            <x-ui.button href="{{ $actionBtnUrl }}" variant="primary" size="sm">
