    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <x-ui.card>
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">Kategori Pengeluaran</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Kelola pos pengeluaran operasional (Listrik, Air, Kebersihan, Perbaikan, dll)</p>
                </div>
                <x-ui.button href="{{ route('expense_categories.create') }}" variant="primary" size="sm">
                        ...
                        <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-700/40 transition-colors">
                            ...
                            <td class="px-4 py-3.5 text-center">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                                    {{ $expense_category->expenses_count > 0 ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300 border border-indigo-200/80 dark:border-indigo-800/60' : 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' }}">
                                    {{ $expense_category->expenses_count }} pengeluaran
                                </span>
                            </td>
                            <td class="px-4 py-3.5 text-right space-x-1.5">
                                <x-ui.button size="sm" variant="secondary" href="{{ route('expense_categories.edit', $expense_category) }}">
                                    Edit
                                </x-ui.button>
